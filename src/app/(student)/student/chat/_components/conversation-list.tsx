// app/(student)/chat/_components/conversation-list.tsx
'use client';
        ...
        <div className="conversation-scroll flex-1 overflow-y-auto">
          ...
          {filteredConversations.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <button
                onClick={onNewChat}
                className="mt-3 rounded-lg bg-[#9d7443] px-4 py-2 text-sm text-white hover:bg-[#b08f5a] transition"
              >
                Start new chat
              </button>
            </div>
...
function ConversationItem({ conversation, isSelected, onSelect, formatTime, index }: ConversationItemProps) {
  const getStatusColor = () => {
    ...
    const participant = conversation.participants.find(p => p.id !== 'student-1');
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isSelected 
          ? "bg-[#9d7443]/15 border border-[#9d7443]/30" 
          : "hover:bg-white/30"
      )}
    >
      <div className="flex items-start gap-3">
        <div className="relative shrink-0">
          {conversation.avatar ? (
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#9d7443]/20 text-[#9d7443]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-stone-500 truncate">
            {conversation.lastMessage.senderId === 'student-1' ? 'You: ' : ''}
            {conversation.lastMessage.content}
          </p>
        </div>
        
        {conversation.unreadCount > 0 && (
          <div className="shrink-0 rounded-full bg-[#9d7443] px-1.5 py-0.5 text-xs font-semibold text-white">
