// app/(student)/chat/_components/search-in-chat-modal.tsx
'use client';
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return parts.map((part, i) => 
      part.toLowerCase() === query.toLowerCase() 
        ? <mark key={i} className="bg-[#9d7443]/30 text-stone-950 rounded px-0.5">{part}</mark>
        : part
    );
  };
          ...
          >
            <div className="bg-gradient-to-r from-[#9d7443]/20 to-[#9d7443]/5 px-5 py-4 border-b border-white/40 shrink-0">
              ...
              <div className="flex items-center gap-2">
                <Search className="h-5 w-5 text-[#9d7443]" />
                  ...
                  <div className="flex items-center justify-between mb-3 shrink-0">
                    ...
                    {searchResults.length > 0 && (
                      <div className="flex gap-2">
                        <button onClick={handlePrevious} className="rounded-full p-1.5 text-stone-500 hover:bg-white/40 transition">
                          <ChevronUp className="h-4 w-4" />
                        </button>
                        <button onClick={handleNext} className="rounded-full p-1.5 text-stone-500 hover:bg-white/40 transition">
                    ...
                    {searchResults.map((msg, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-medium text-[#9d7443]">{msg.senderName}</span>
