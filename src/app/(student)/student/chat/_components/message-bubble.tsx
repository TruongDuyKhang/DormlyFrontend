// app/(student)/chat/_components/message-bubble.tsx
'use client';
    ...
    >
      ...
      {showAvatar && (
        <div className={cn(
          ...
          isOwn ? "bg-[#9d7443] text-white" : "bg-stone-300 text-stone-700"
        )}>
      ...
      <div className={cn(
        ...
        isOwn 
          ? "bg-[#9d7443] text-white rounded-tr-sm" 
          : "bg-white/60 text-stone-800 rounded-tl-sm"
      )}>
        {!isOwn && showAvatar && (
          <p className="mb-1 text-xs font-medium text-[#9d7443]">
            {message.senderName}
          </p>
        )}
